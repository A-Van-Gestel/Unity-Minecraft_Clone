...
### ~~21. No player effect~~ (`FLUID_BUGS` #02)

**Severity:** Missing Feature (visual only)  
**Fixed:** September 2026  
**Files (physics half):** `Player.cs`, `Physics/VoxelRigidbody.cs`, `World.GatherFluidContact`  
**Files (visual half):** `Shaders/UnderwaterOverlay.shader`, `Rendering/UnderwaterOverlayRendererFeature`,
`Rendering/SubmersionOverlay`, `World.GatherEyeSubmersion` / `PublishSubmersionGlobals`,
`Helpers/EyeSubmersion`, `Helpers/FluidSurfaceResolver`
...
**Closed `UW-6`, confirmed in game 2026-09-05.** Lava got its feel pass and is authored to a hot orange,
`submersionColor` sRGB `(0.85, 0.30, 0.05)` at density `1.5` — sinking into a pool now loses sight of the
surface. Two code defects surfaced with it and were fixed in the same phase: the authored tint was consumed
as **linear** while being authored as sRGB, so every fluid rendered lighter and less saturated than its
swatch (lava's deep red arrived salmon; `SubmersionOverlay.Pack` now converts, pinned by `B26`), and the
globals were published a frame late from `World.Update` (now from
`RenderPipelineManager.beginCameraRendering`, confirmed at a 20 fps cap where the lag is actually visible).
Water's confirmed look was preserved rather than re-tuned, by re-expressing its authored value in sRGB.

**Two known limitations were accepted, not fixed — neither reopens this entry:**

- ⏸️ **The waterline stays a hard edge — `UW-5` paused 2026-09-05, on cost/benefit rather than closed.**
...
Guarded by `Minecraft Clone/Dev/Validate Underwater Render` (`B1`-`B26`). The authored values themselves
are **not** baselined and cannot be: the suite builds its own densities and colors and reads nothing from
`BlockDatabase.asset`, so a retune can neither red it nor be vouched for by it.
