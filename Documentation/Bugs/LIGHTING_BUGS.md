...
**Confidence:** Medium — the seeding gap is verified by code inspection, but the in-game impact may be masked by the fluid simulation re-triggering light updates on lava flow.
...
**Severity:** High
**Status:** Open / Root cause identified by code inspection, awaiting instrumented in-game confirmation
**Confidence:** High — the destructive seeding path is verified by code inspection and matches all three reported symptoms (static cut-off, border flicker, correct-after-reload-until-next-update).
...
**Status:** Open / Root causes identified by code inspection, awaiting instrumented in-game confirmation
**Confidence:** High for the unloaded-neighbor path; Medium for the in-flight-job race (timing dependent).
