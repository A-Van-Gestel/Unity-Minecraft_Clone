...
**Severity:** Medium
**Status:** Open — faithfully reproduced **synchronously** in the validation harness as known-bug scenario **K12a** (June 2026); fix + in-game confirmation TODO.
...
**Validation suite (June 2026):** unlike Bug 05 / Bug 09 (not synchronously reproducible), this **does** reproduce deterministically in the synchronous harness, now captured test-first as known-bug scenario **K12a** (`LightingValidationSuite.KnownBugs.cs`, run via `Minecraft Clone/Dev/Validate Lighting Engine`). K12a builds a 1-wide roofed corridor straddling the `x15|16` seam, lit by a single sky shaft that opens **both** shared seam columns (so the two seam voxels are mutually equal at sky 15 — each appears lit "by" the other). After initial
convergence (matches the oracle), it roofs both seam columns — one `PlaceBlock` per chunk, so both seam chunks carry a sunlight column recalc into the **same** wave — and runs the grid **wave-parallel** (`RunWaveToConvergence`, production's concurrent-job / schedule-time-snapshot model). The field converges to a **stable** state (one wave, no residual work — the static defect, not Bug 11's oscillation) that does **not** match the oracle: the seam pins at its pre-roof value minus one (14) and stays bright downstream while the borderless oracle is dark.
K12a asserts the *correct* behavior (`MatchesOracle` after removal), so it is **RED today** and will flip green with no test edits once a fix lands; it then promotes to a baseline. The fix is still TODO — candidate directions: cross-seam removal-loop detection (seed the removal from both seam columns), or a relight trigger that does not depend on a single removal initiator. (A single-side shaft + single roof edit, or sequential `RunToConvergence`, does **not** reproduce — the neighbor snapshots the already-dropped value; the simultaneous same-wave
perturbation of both seam chunks is required, mirroring how B48 forces the sibling Bug 11.)
