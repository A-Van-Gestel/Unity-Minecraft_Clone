...
**Validation suite (June 2026):** Fifteen repro attempts total across five harness layers:
...
- **Frame simulator, combined stress (B29):** The definitive test — layers ALL harness capabilities simultaneously into one scenario modeling heavy ocean biome load: underwater environment, single-slot budget, multi-frame held flights, shuffled completion+scheduling, fluid-flow contention, two rapid break+fluid+place cycles before any flight completes, both chunks in-flight with interleaved release. 50 RNG seeds. All converge to the oracle.

Every production scheduling behavior modelable in the synchronous harness has been exhausted. The Bug 07/08 cross-chunk mod delivery fixes were already present when Bug 09 was last observed — the bug is either a genuine async race condition (Burst job system timing, IL2CPP memory ordering) that synchronous `.Run()` cannot reproduce, or is no longer present in the current codebase. A faithful failing repro is still TODO before this bug's fix can be test-driven; the 29 baselines serve as comprehensive regression guards.
