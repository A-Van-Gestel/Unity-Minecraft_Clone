...
## 03. `WorldData.GetLocalVoxelPositionInChunk` uses `(int)` cast instead of `FloorToInt`

**Severity:** Bug (latent)  
**Files:** `WorldData.cs` — `GetLocalVoxelPositionInChunk`

~~The method uses `(int)` cast which truncates toward zero, producing wrong results for negative coordinates.~~

> **Status:** Fixed (March 2026). Both `x` and `z` components now use `Mathf.FloorToInt()`. The `y` component retains `(int)` cast since Y is always non-negative.
