...
- **Surface-relative worm suppression** --- `depthFadeMarginBottom`/`depthFadeMarginTop` fade carving against the fixed `MinHeight`/`MaxHeight` config bounds, not the actual terrain surface. Since terrain height varies per column (hills, valleys, mountains), a fixed `MaxHeight` cannot produce "rare surface breakthroughs." If `MaxHeight=78` and terrain is at 72, there are only 6 blocks of fade --- worms punch through easily. If terrain is at 85, the worm never reaches the surface at all. A surface-relative suppression system would compare the worm's Y
  position against each column's actual terrain height (from the heightmap) and taper the carving radius as `pos.y` approaches `surfaceHeight - margin`. This would allow natural, occasional cave openings while preventing the current over-carving of the surface. Implementation requires passing the terrain heightmap (or a coarse approximation) into the `StandardWormCarverJob`, which currently has no surface data. The heightmap could be a low-resolution 2D lookup (one value per column) sampled at the worm's XZ position each step. A configurable
  `surfaceFadeMargin` (e.g., 8-16 blocks) would control how far below the surface the fade begins.
