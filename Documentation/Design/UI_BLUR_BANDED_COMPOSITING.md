...
| Canvas gamma flag           | `m_VertexColorAlwaysGammaSpace` is **0 in `World.unity:4637` but 1 in `MainMenu.unity:3258`** — the two scenes already disagree about how UI vertex colours are treated. Pre-existing and unrelated to this design. **UB-6 landed without it mattering** — the converted `MainMenu` was checked against an older production build and the colors match, so the flag is left alone rather than reconciled. Both carry `m_AdditionalShaderChannelsFlag: 25` (Tangent + Normal + TexCoord1). |
