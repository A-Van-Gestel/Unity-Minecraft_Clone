...
| P-4 | ✅ Backpressure (§3) **COMPLETE**: §3.1 cap + §3.2 discard + §3.3 unload-via-persistence SHIPPED 2026-07-21; **§3.4 time budgets (rate quota + ms ceiling) + §3.5 panic gate (+ §5.3 draw-drain rider) SHIPPED 2026-07-23** — fill-load FPS 13.3→29.1, hitch frames 67%→11% (editor screening, [benchmark](../Performance/CHUNK_PIPELINE_P4_BACKPRESSURE_2026-07-23_BENCHMARK.md)); **IL2CPP GO (final)** ([player A/B](../Performance/CHUNK_PIPELINE_P4_BACKPRESSURE_IL2CPP_2026-07-23_BENCHMARK.md)): legacy never drains post-relocation (3/3 legs 300 s timeout) vs ON 15.5 s / 2.2% hitches / 78 ms worst frame — **all three flags retired 2026-09-06**, the family is now unconditional. Refinement: FPS-cap-proportional ceilings (`scaleBudgetCeilingsWithFpsCap`, default-ON) so a voluntarily capped 30/15 FPS session is not over-throttled — B7 pinned, **IL2CPP GO** ([A/B](../Performance/CHUNK_PIPELINE_P4_CEILING_SCALING_IL2CPP_2026-07-23_BENCHMARK.md)): 30-cap fill ×1.82, 15-cap ×1.32, zero frame-health cost |   ✅   | 🟡→🔴 |   🟢    |  ✅  |    ✅     |
...
retired 2026-07-23) and LI-2 (1 flag, retired 2026-07-24) are the worked precedents. Retire in one
atomic pass per family, not one flag at a time.

**The census is currently empty — every shipped rollback flag has been retired.**

| Flag (`Settings`) | Family | Shipped | Evidence | Retired |
|---|---|---|---|---|
| `enablePipelineTimeBudgets` | P-4 §3.4 | 2026-07-23 | [IL2CPP A/B](../Performance/CHUNK_PIPELINE_P4_BACKPRESSURE_IL2CPP_2026-07-23_BENCHMARK.md) — **GO (final)** | 2026-09-06 |
| `scaleBudgetCeilingsWithFpsCap` | P-4 §3.4 refinement | 2026-07-23 | [ceiling-scaling A/B](../Performance/CHUNK_PIPELINE_P4_CEILING_SCALING_IL2CPP_2026-07-23_BENCHMARK.md) — **GO (final)** | 2026-09-06 |
| `enableGenerationPanicGate` | P-4 §3.5 | 2026-07-23 | same P-4 A/B | 2026-09-06 |
| `enableConvergentEdgeCheckCascade` | P9-2 | 2026-08-02 | [P9-2 capture](../Performance/CHUNK_PIPELINE_P9_2_CASCADE_IL2CPP_2026-08-02_BENCHMARK.md) — **GO** | 2026-09-06 — it was also a Performance-tab toggle, so its retirement removed a settings option |
...
**Three notes that cost a session each to learn.** (1) The P-4 harness
(`Benchmarks/P4BackpressureBenchmark.cs`) is **kept**, not deleted, when its flags go, following
LI-2's FullHeight oracle and TG-4's pruned harness. It was *not* collapsed to a single configuration
as this section previously prescribed: retiring `scaleBudgetCeilingsWithFpsCap` would have left the
matrix with no varying axis at all, so the harness was retargeted to sweep the **frame cap**
(uncapped / 30 / 15), which is the axis the retired refinement was built for and the one that still
changes pipeline behavior. Prefer retargeting a harness onto a surviving axis over flattening it.
Its reports are now written as `P4BackpressureFrameCap_*`; captures from before 2026-09-06 are
`P4BackpressureAB_*` and carry a genuine budgets-vs-legacy control leg that the harness can no
longer produce.
(2) A rollback flag must be listed in `SettingsManager.OverlayBenchmarkSettingsFromDisk` or it
...
- **⚠️ …but *analytic per-pixel* AO does retire constraint (a), and it is now the endorsed direction (added 2026-08-09).** The bullet above argues from **interpolation**, and that argument does not reach a shader that evaluates AO **analytically** from the occupancy volume: each fragment derives occlusion from its own cell's neighbourhood, so a merged quad spanning many cells is shaded correctly by construction. `SILHOUETTE_CONTACT_SHADOWS.md`'s **D7** was decided (2026-08-09) as "build `SS-3` now, per-pixel on `VX-1` as the destination" — so **constraint (a) has a named, owner-endorsed escape hatch**, on the same substrate as route (b) but consuming both of VX-1's volumes rather than the filtered light one. Until it lands, constraint (a) stands as worded above.
- **The interlock runs both ways, and neither side may bank the other (added 2026-08-09).** VX-1's volume wants to grow with view distance to avoid an AO horizon, and uniform-resolution growth is **quadratic** (9.4 MB at view distance 5 → 150 MB at 20 — table in that entry). This item's 60–90 % vertex cut is the obvious thing to pay for it with. Recorded because the owner raised it, and flagged because it is not yet real on either side: this item is gated on its own design doc and on harness gap **MH-8**, the AO escape hatch is gated on VX-1 existing, and the two costs sit in **different budgets** (mesh memory + per-frame upload versus a resident 3D texture). Price it when both are built, not before.
