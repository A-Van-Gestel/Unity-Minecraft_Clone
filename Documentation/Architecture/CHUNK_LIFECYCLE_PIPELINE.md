...
writes the staged snapshot synchronously. Deterministic failures (zero-length serialization) are `FailedPermanent`: released loudly, never retried.
Guarded by `Minecraft Clone/Dev/Validate Save Durability` (B1–B12, dev-only `InjectSaveFaults`/`InjectZeroLengthSerializes` seams); see the CP doc §4.3/§7 CP-6 and the storage doc §5.1.
