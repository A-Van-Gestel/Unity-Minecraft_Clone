...
cardinals via face culling, diagonals via fluid corner geometry, and the shared acquire-site offset table), MH-13 added **B40** (the same guard for the eight neighbor **light** maps), **VO-5** added **B41–B43** (fractional ambient occlusion for partial blocks), and **VO-6** added **B44–B45** (sub-block face light sampling, promoted from the `KM01a`/`KM01b` repros of the now-archived Bug M01) — **tip is B45 (45 baselines); see §4 for the arc detail.** The suite currently has **no open known-bug scenarios**: `MESHING_BUGS.md` Bug M02 is filed without a repro.  
