...
cardinals via face culling, diagonals via fluid corner geometry, and the shared acquire-site offset table), MH-13 added **B40** (the same guard for the eight neighbor **light** maps), **VO-5** added **B41–B43** (fractional ambient occlusion for partial blocks; `B41` was retargeted onto the silhouette primitive by `SS-3a`), **VO-6** added **B44–B45** (sub-block face light sampling, promoted from the `KM01a`/`KM01b` repros of the now-archived Bug M01), **VO-8** added **B46** (per-corner octant occlusion), the Bug M03 follow-up added **B47** (a recessed partial block must not occlude its own mid-plane face), and the **Bug M02** fix added **B48** (a mid-plane face is culled by the cell it looks into, not by the block-boundary neighbor — promoted from the `KM02` repro), **VO-9b** added **B49** (a face a partial occluder can reach is subdivided, the subdivision is gated, and it still renders its own corner field), and **`SS-0`** added **B50** (custom-mesh fixtures' authored bounds match their flattened geometry — its second leg, the post's non-monotonic *coverage* sweep, was removed by `SS-3a` with the function it tested), **`SS-2`** added **B56** (a face corner with 0/1/2/3 fully-occluding neighbors still reads exactly 255/191/64/64 after the occlusion function was replaced), and **`SS-2a`** added **B57** (the corner seal is at full strength in the corner and falls off away from it — the first scenario to read the field *between* a face's corners and its centre) and **B58** (a cell the seal hides does not also feed the light average — the first scenario to shade under a **non-uniform** light field, see MH-3), and **`SS-3`** added **B54** (a lone full cube's shadow follows Euclidean distance to its silhouette — the suite's first **metric** assertion, asserting the shape of the field around an isolated block) and **`SS-3a`** added **B59** (a straight wall's shadow is uniform along the wall — no scalloping at the cell seams) and **B60** (a partial occluder that casts nothing does not subdivide the face it stands over), and **`RF-3`** added **B61** (an emitter stamps its emission into vertex-colour alpha while a non-emitter stays at 0 — see §4's note on why its control half is the load-bearing one) — **tip is B61, 57 baselines; see §4 for the arc detail.** The suite currently has **no open known-bug scenarios**: `MESHING_BUGS.md` Bug M04 is filed without a repro.  
**Created:** 2026-06-16  
...
- **Smooth-lighting *values* (Wave 2, MH-3).** `MeshingTestWorld.FillLight` + `Run(SmoothLightingQuality.High)`
  populate a uniform light field; `MeshOracle.ExpectedUniformCornerLight` (hand-derived `17·V`, LUT-independent)
    + `MeshAssert.LightDataMatches` pin the smooth-light encoding (B11: full sky 255, intermediate 119/51).
...
- **Closed by:** `MeshingTestWorld.FillLight`/`SetLight` populate the in-chunk light map and `Run(SmoothLightingQuality.High)`
  ...
  `CornerOffsets` LUT, avoiding the A4 shared-assumption trap. `MeshAssert.LightDataMatches` + **B11** pin two configs: full skylight (→ 255 sky) and an intermediate, multi-channel blocklight (R=7→119, G=3→51, proving averaging + UNorm8 rounding + channel order, not a vacuous all-zero/saturated read), with an A≠B positive control proving the populated map drives the output.
...
- **Effort:** 🟡 medium.
- ⚠️ **The deferred half of this entry shipped a visible defect (2026-08-09, `SS-2`/`SS-2a`) — treat "uniform light only" as an active blind spot, not a footnote.** Every AO scenario in the suite fills light with `FillLight(uniform)`, and **under a uniform field a weighted mean equals that value no matter how it is weighted** — so an entire class of defect is invisible here: any error in *which cells* the light average draws from, or in *how* it weights them. `SS-2` shipped exactly that (it averaged over cells that "hold light" instead of over cells the point can see, so a corner-sealed **air** cell was counted as occluded *and* averaged in), 436 baselines stayed green, and the artifact was obvious in game on plain full cubes — corners up to **twice as dark** as the model claims. **B58** is the first meshing scenario to shade under a non-uniform field: it holds one cell's light different from the rest and asserts a *sealed* corner ignores it while an *open* corner tracks it.
- **Rule for new AO scenarios:** if what you are asserting depends on the light average's cell **set or weights**, a uniform field cannot see it. Vary one cell. Reserve uniform fields for assertions about encoding and occlusion magnitude, which is what B11 and B56 legitimately use them for.
...
- **Widened 2026-08-09 (`SS-0`), and the reason is a fidelity finding in its own right.** Every custom-mesh fixture was a **full-width box** — `AppendBoxMesh` was parametric on height alone — so the palette had no shape whose occlusion of a neighbouring face a two-corner blend cannot reproduce. That is precisely why `VO-9b` could ship a subdivision substrate that changed nothing while the suite stayed green: *the fixtures could not tell*. The builder now takes full `min`/`max`, and a **`Post`** entry (id 9, `Count` 10) is a quarter-cell column with four faces interior to its own cell. Measured on the new fixture: a post standing on a face darkens it by **~3 %** (255 → 251/247) where a slab manages 25 %.
- **Dead-code deletion, and what it cost the suite (`SS-3a`, 2026-08-09).** `SS-2` replaced the coverage-fraction occlusion query with a silhouette distance field, which left `AmbientOcclusionOctantCoverage` / `AmbientOcclusionRegionCoverage` (and their `GetOctantCoverage` / `GetRegionCoverage` backings) with **no production caller** — alive only because two baselines still read them. Both were deleted. The two baselines were handled differently, and the distinction is the reusable part:
    - **`B41` was retargeted, not deleted.** Its claim — a block without custom bounds occludes all of its cell or none of it, on every face and orientation — is still a live guarantee; only the function expressing it changed. It now sweeps `AmbientOcclusionPlaneSilhouette`. Measured, neutering the opacity gate reds it *and ten other baselines* — so it is not a unique guard, and the docstring says so. Its distinct value is that it reads the **primitive directly** (the failure names the block, plane and metadata byte rather than a shifted light value) and sweeps the **whole palette**, where the shading baselines only ever place three block types.
    - **`B50`'s second leg was deleted.** Its subject was the coverage function's *own* behaviour (finding S9: a coverage fraction is not a distance field, because `GetRegionCoverage` normalized by the query region's volume). That finding is what justified abandoning coverage, and it is written down — re-proving a property of deleted code on every run guards nothing. Leg 1 (F13 bounds-match-geometry) is untouched.
    - **The rule:** when a baseline's *subject* disappears, ask whether the **claim** survives on the new code path. If it does, retarget it; if the claim was about the removed mechanism itself, delete it and make sure the finding it produced lives in a document.
- **The F13 rule is now structural, not asserted.** A fixture's geometry and its authored `collisionBounds` are built from **one `BlockCollisionBounds` value used twice** (`TestCustomMeshLibrary.HalfSlabBounds` / `PostBounds` → `TestMeshBlockPalette.MakeCustomBox`), so the divergence F13 recorded — a slab in geometry, a full cube in shape, invisible until some phase first asked — cannot be expressed. Baseline **B50** still checks it end-to-end against a future hand-edit that reintroduces a second source.
- **Every AO scenario was a *point* scenario until B59 (`SS-3a`, 2026-08-09).** They place geometry and read values at chosen positions, which cannot see a defect whose signature is **periodicity** — shading that varies with where the *grid lines* fall rather than with the geometry. A straight wall scalloped 31 light units between its cell seams and its cell centres, in a suite of 54 baselines, because reading one wall-adjacent point tells you nothing about the point half a cell along. **B59 is the first invariance scenario**: it sweeps a row and asserts the values are *equal to each other*, with no expected value at all. That shape is reusable — whenever geometry is uniform along some direction, assert the shading is too, and the assertion survives every retuning of the model that produced it.
- **A whole-field probe is not the same as a probe that reads the whole field (`SS-2a`, 2026-08-09).** `TopFaceSubVertexField` existed, and `SS-2` still shipped a defect visible at a glance in game with all 435 baselines green: every scenario read a face's **corners** or its **centre**, and the artifact — a dark wedge running diagonally out of concave corners — lived in the field *between* them. **B57** is the first scenario to sample there, and its shape is the transferable part: a **four-configuration differential** (both walls / either alone / neither) cancels the falloff profile, the radius, the fixture that trips the tessellation gate and the light field, leaving only the interaction under test. Reach for that construction whenever an assertion would otherwise have to hard-code a value that a tuning surface can legitimately move.
- **New probe: `TopFaceSubVertexField`** (`MeshingValidationSuite.SubCellShading.cs`) returns every emitted vertex on a face keyed by its **position within that face**, so a reading is independent of whether the face is one quad or N×N sub-quads. It is the whole-field counterpart to `TopFaceCornerSky`; between them, no meshing probe need ever index by quad order again — the assumption that broke B42/B46 when `VO-9b` landed.
- **A prove-red proves the assertion is wired, not that the assertion is right (`RF-3`, 2026-08-12).** **B61** was written alongside the emissive-alpha stamp and passed; both prove-red mutations reddened it, so the guard looked sound. It was still wrong. Its control asserted that a non-emitter keeps the historical `255` fill, which was what the code did — but the shader reads that channel as emissive *strength*, so `255` means "fully emitting" and every ordinary block in the world rendered at full boost. The suite could not see it: **57 baselines green, and the terrain visibly washed out in the first capture.** Worse, the mutation that removed the non-emitter guard — reported as a successful prove-red — was actually producing the **correct** output, and B61 called it a failure.
    - **The rule:** a prove-red only demonstrates that the assertion *reads* the value under test. It says nothing about whether the expected value is the right one, because both the assertion and the mutation are derived from the same reading of the implementation. When the expected value encodes a **contract with a consumer this suite does not run** (here, the shader), the suite cannot close the loop — only the consumer can. Assert against the consumer's requirement, and get eyes on the real output before calling such a guard sound.
...
2. **MH-3 — smooth-lighting *value* oracle** (completes MR-2; prereq MR-8).
    ...
    - *Baseline:* a deliberately trivial, hand-computable lit config (e.g. one sky-exposed top face = full skylight; a face against a lamp = known blocklight) so expected values are derivable by hand.
