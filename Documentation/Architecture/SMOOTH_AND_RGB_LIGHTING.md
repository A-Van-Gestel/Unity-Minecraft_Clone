...
Custom meshes use axis-aligned face culling identical to standard cubes (each custom mesh face maps to one of 6 face directions). Culling still consults the **block-boundary** neighbour even for a face that sits inside its own cell, which is `MESHING_BUGS.md` **Bug M02** — open.
...
The bilinear approach maps each vertex's rotated block-local position to (u, v) on the world face's perpendicular axes via `GetCornerUV`, then blends the 4 corner lights with `BilinearLerpLight`, so narrow meshes receive position-weighted gradients rather than hard corner snaps.

**Which cell a face samples (VO-6).** The blend above distributes light *within* a face; it says nothing about where that light came from. Until VO-6 the source quad was always taken from `pos + rotatedOffset` and the ambient ring around it — correct only for a face lying on the cell boundary. A half slab's large face sits at the mid-plane, half a block inside its own cell, so it was lit by a cell a full block past the surface it belongs to (`MESHING_BUGS.md` Bug M01).
...
`LightData` stores per-vertex light values using `Color32` (4 bytes, maps to UNorm8x4). `InterleavedStream3` is populated by `MeshPostProcessJob` in `Chunk.cs` (see Section 2.6.5) to combine normals and light data into a single GPU-uploadable buffer.
...
| `Chunk.cs`                                          | Add `MeshPostProcessJob` (Burst) for Normal/LightData interleaving, thread `InterleavedStream3` through `ApplyMeshData`                                                                                                                                                                                                                                                                                                                  |
