...
| Physics                 | Fully custom (`VoxelRigidbody`), no PhysX world-space dependency. Operates on `transform.position`; `World.CheckPhysicsCollision:3373` floors the Unity-space AABB straight into `TryGetVoxel(int)` lookups — which resolve **populated** chunks only (Fluid §18), so an unpopulated placeholder collides as empty just like an absent chunk. `ClampToWorldBorder` clamps against ±`BorderRadius`.                             |
| Interaction             | `PlacementController.MarchRay` traverses camera-space floats through `VoxelRayDDA`, which floors the ray origin once and then steps in integer cells; cells feed `VoxelMod.GlobalPosition` (**persisted** to `pending_mods.bin`) and the highlight/place transforms.                                                                                                                                                                                                                                |
...
4. `AssertPlayerNearOrigin` (`[Conditional("UNITY_ENABLE_CHECKS")]`, latched): player Unity-space XZ ≤
...
| Raycast/placement (`PlacementController.MarchRay/Probe/CanPlaceAt`)                     | Traverse in Unity space; convert resulting integer cells (`hitCell`, `placeCell`) — **fragility hotspot**, see §7 gates                 |
...
| Version | Extension                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
|---------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **v2**  | ✅ **SHIPPED + IN-GAME CONFIRMED 2026-07-20** — generation noise-precision rider (`WORLD_SCALING_IMPLEMENTATION.md` §6): FNL `Precise64` double coordinate pipeline behind the global "Far Lands (Classic Noise)" setting (default precise; deviation from the world-version-gating spec — user decision). Extends the usable radius from ~16.7M to the permanent ±2³¹ edge — verified in-game: terrain normal even at the integer-limit world border. Classic pipeline (and its Far Lands) preserved bit-identically as the opt-in mode. |
...
- ~~**Terrain generation still degrades at ~±16.7M voxels**~~ — **closed 2026-07-20 (v2 rider
  shipped + in-game confirmed, terrain normal even at the ±2³¹ edge)**: `FastNoiseLite` gained a `Precise64` double
...
- **Two coordinate sites still ROUND where the engine otherwise floors** — left untouched by WS-4a
  ...
  conversion (`WorldOrigin.UnityToVoxelCell`, `VoxelRayDDA` behind `MarchRay`, `CheckPhysicsCollision`) floors. The survivors are
