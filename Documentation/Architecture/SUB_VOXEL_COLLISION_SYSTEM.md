...
- **This model is becoming the engine's single shape source of truth.** [`../Design/VOXEL_OCCLUSION_REFACTOR.md`](../Design/VOXEL_OCCLUSION_REFACTOR.md) (**`VO-*`**, proposed) derives *per-face light occlusion* from the same authored `BlockCollisionBounds` and the same rotation path, rather than minting a second rotation-aware shape descriptor for lighting (its §4 D1 records why). Two consequences for this doc: **`VO-1`** mirrors the bounds into `BlockTypeJobData` and moves the rotation-to-AABB core into a Burst-safe shared home that `BlockCollisionBoundsUtility` then calls (behavior-preserving, guarded by `NS-4`), and the single-AABB limitation above is inherited verbatim by lighting — so **`VQ-4`** now unblocks compound *occlusion* as well as compound collision. `VO-1` is asked to shape its utility around a bounds list internally so `VQ-4` does not have to re-cut that seam.
