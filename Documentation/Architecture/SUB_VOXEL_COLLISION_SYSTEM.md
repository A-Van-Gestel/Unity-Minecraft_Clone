...
**Status:** Implemented — core runtime collision, placement API separation, Block Editor authoring, editor preview, and in-game collision-bounds debug visualization are fully implemented and playtested, and the solver is guarded by the **`NS-4`** physics validation suite (`Minecraft Clone/Dev/Validate Physics Solver`, 44 baselines) as of 2026-09-03. **Target Engine:** Unity 6.4+  
...
| 2026-09-03 | **Fluid contact added to the solver** (`FLUID_BUGS` #14 + #02's physics half): a new §7 bullet records that collision stays fluid-blind while a separate `World.GatherFluidContact` resolves buoyancy, drag and flow push, applied in `CalculateVelocity` rather than per-substep. The flow vector is not a second implementation — `VoxelMeshHelper`'s corner-flow core moved to `Jobs.BurstData.BurstFluidFlowUtility` and both meshing and physics call it, so the current a body feels is the one the shader draws (physics negates it: the meshing vector is a UV scroll offset pointing uphill). Suite 26 → 33 baselines (`B27`–`B33`, `B27` the positive control); the status line's count and date follow. |
| 2026-09-03 | **Fluid feel pass after the first in-game session.** Four defects, all in §7's new bullet: the swim stroke hard-set vertical momentum (overwriting buoyancy and drag, launching the body clear of the pool where it skipped across the surface) and is now an acceleration toward a **submersion-scaled** target, so a swimmer settles at the waterline; the stroke gained a downward direction (crouch), which had been missing outright; `buoyancy` 0.9 → 0.55 and `swimAscendSpeed` 3 → 0.9 on both shipping fluids, taking the sink rate from ~0.2 to ~1.0 units/s; and a falling column now pushes **down** rather than forwarding the renderer's outward vector. Suite 33 → 35 (`B34` descend, `B35` the falling column). |
| 2026-09-03 | **Second fluid feel pass.** Three defects from the follow-up session, all about forces that could not be fought: the **step-up pre-pass** was gated on `IsGrounded`, and a floating body is never grounded, so leaving a pool was impossible — buoyancy now counts as support for that pre-pass, and pressing into the bank steps out of the water exactly as it climbs a slab; the **falling current** was applied as displacement, bypassing `_verticalMomentum` entirely, so no swim stroke could touch it and a waterfall was a one-way trip down — it is now a momentum target with an acceleration below `SWIM_ACCELERATION`, and the difference between the two is the rate a swimmer climbs one; and the **horizontal speed penalty** scaled linearly across the whole collider, so a body at the surface (where submersion is small by construction) kept ~76% of its walk speed and slid across the top — it now ramps to full strength by `FULL_HORIZONTAL_DRAG_SUBMERSION` (a quarter of the body). Suite 35 → 38 (`B36` climbing a column, `B37` the wading penalty, `B38` stepping out onto a bank). |
| 2026-09-03 | **Review follow-up.** A code review of the whole change turned up two real defects. `Player` reported the swim intent only in the `else` of its grounded-jump branch, so the intent went **stale**: climb out holding jump and it stayed latched at `+1` across dry land, applying a stroke on the first tick back in the water — both input reports are now unconditional. And the teardown guard in `GatherFluidContact` tested `NativeArray.IsCreated`, which **stays true after disposal**: these types dispose hoisted *copies*, so each `readonly` field keeps its pointer while the memory is freed (measured — a read then throws `ObjectDisposedException`). The same mistake made their own `Dispose` non-idempotent, so `JobDataManager` and `FluidVertexTemplatesNativeData` gained an `IsDisposed` flag that fixes the guard and the double-dispose together. `B44` covers it. Lava also got a first pass at feeling thicker than water (drag 10, speed x0.3, push 0.7, ascend 0.5, buoyancy 0.7); a full tuning pass is still owed. Suite 43 → 44. |
| 2026-09-03 | **The climb-out jump was an ordering race, and the harness could not see it.** Leaving the water still launched the body ~2.24 blocks for a ~1.04 climb. Cause: the step-up sets `IsGrounded` *within its own tick*, so the input layer saw a grounded body and latched a jump one render frame BEFORE the next tick's fluid-exit edge armed the block meant to stop it — the exit edge is a tick too late by construction. The block is now armed inside the step-up itself. Two testing lessons are recorded because both cost a round: `PhysicsTestWorld.Tick` reproduced `FixedUpdate` as "resolve, then move" and silently **omitted the jump application entirely**, so no jump could ever fire in the suite and every jump assertion was untestable — the block is now extracted as `VoxelRigidbody.ApplyPendingJump` and the harness invokes the real code. And `B41` asserts a request is not *latched*, which cannot observe an ordering that latches it a tick early; `B43` asserts the end-to-end outcome (peak equals final, jump counter unchanged) and reddens where `B41` stays green. Suite 42 → 43. |
| 2026-09-03 | **Climbing out made player-driven.** The climb inherited the slab step-up's trigger — walk into it and it fires — which for a movement of roughly a block was both surprising and exploitable: a body falling back toward the pool, still partly submerged, could brush the bank and be launched out again indefinitely. It now also requires an upward swim intent (`B42`), and only a floating body gets the swim allowance at all, so wading onto a slab steps like walking. The timed post-fluid jump delay was replaced by `requireJumpReleaseAfterFluid`: a fixed delay only postponed the jump, which still fired while the button stayed down, and climbing out *is* holding the button. `swimStepHeight` 1.25 → 1.45. The `PlayerBody` mesh is hidden from its own camera — it casts no shadows and step smoothing can displace the camera far enough to see it from outside. Suite 41 → 42. |
| 2026-09-03 | **Getting out of the water.** Two in-game findings, both about the exit. The step-up pre-pass used the walking `stepHeight` while swimming, but a floating body's feet sit `submersion x height` (~0.92) under the surface, so a one-block bank stands ~1.04 above them — the pre-pass could never fire and a swimmer was stuck until they found a one-deep section. `swimStepHeight` (1.25) applies while in fluid; because the shortfall is measured from the surface rather than the bottom, it is depth-independent and still refuses a two-block wall. Separately, climbing out means *holding* jump, so the body jumped on the frame it landed and the step-up and the jump fused into one launch — `jumpSuppressionAfterFluid` (0.3 s, edge-triggered on leaving fluid) defers it. Suite 40 → 41 (`B41`); `B38` rebuilt around a settled float beside a real one-block bank, and now asserts the bank is out of walking reach so it cannot pass on the ordinary step height. |
| 2026-09-03 | **Step smoothing, and a climb floor for waterfalls.** Two in-game findings. The step-up pre-pass snaps the collider vertically in one tick, which a half-slab gets away with and a waterline does not — `VoxelRigidbody.stepSmoothing` (seconds, 0 disables, per body) now records the snap as a **view** offset that eases to zero on the render clock, and `Player` subtracts it from the camera's local height. The collider still snaps, deliberately: a body that climbed gradually would spend those frames inside the block it is climbing. `B40` pins that the resolved displacement AND the resting position are identical with smoothing on and off. Separately, swimming up a falling column still lost to the current under the shipping tuning (~0.21 m/s of sinking), so `WATERFALL_CLIMB_FLOOR` guarantees a climb at 35% of the still-water stroke speed — a stated gameplay affordance, since any honest force balance nets downward there and an unclimbable waterfall is an invisible trap. Suite 39 → 40. |
| 2026-09-03 | **Float height pinned** (`B39`). The surface-skimming above was two defects, not one: the horizontal penalty's ramp *and* the height the body floats at. A swim stroke's per-tick authority is worth about as much as one tick of gravity, so while it kept full strength as the body rose it cancelled gravity at any depth — the body settled **0.094** submerged, riding on top of the water where no penalty ramp could have helped. Scaling the stroke's authority by submersion, not just its target, moves the balance under the surface. The float height is emergent from buoyancy, drag and stroke speed together, so `B39` asserts a band rather than a value; every individual force was already correct while the equilibrium they produced was wrong, which is the class of defect no per-force baseline catches. |
...
- **This model is now the engine's single shape source of truth.** [`../Design/VOXEL_OCCLUSION_REFACTOR.md`](../Design/VOXEL_OCCLUSION_REFACTOR.md) (**`VO-*`**) derives *per-face light occlusion* from the same authored `BlockCollisionBounds` and the same rotation path, rather than minting a second rotation-aware shape descriptor for lighting (its §4 D1 records why). **`VO-1` shipped 2026-08-07:** the 8-corner rotation formerly inline in `BlockCollisionBoundsUtility.GetRotatedBounds` now lives in `Jobs.BurstData.BurstOcclusionUtility.RotateLocalBounds` (Burst-safe), and this utility delegates to it and only re-spaces the block-local result onto the caller's cell — §3.2's math is unchanged, its home moved. The bounds are also mirrored onto `BlockTypeJobData` (`HasCustomBounds`/`BoundsMin`/`BoundsMax`) so jobs can query block shape. The single-AABB limitation above is inherited verbatim by lighting, so **`VQ-4`** now unblocks compound *occlusion* as well as compound collision.
- ⚠️ **`NS-4` does not discriminate the rotation path.** Proven 2026-08-07 by `VO-1`'s prove-red: with the shared rotation core transposed, **all 26** Physics Solver baselines stayed green — none of them distinguishes a rotated custom-bounds volume from its inverse. Any future change to `GetBounds`/`RotateLocalBounds` is therefore **not** protected by this suite; today the only guard is `Minecraft Clone/Dev/Validate Occlusion` baseline `B2`. Closing this gap (e.g. land a body on a *vertical* slab and assert the rest height differs from the identity orientation) is `NS-4` work tracked in the VO doc's §7 roadmap.
...
- **Fluids are excluded from *collision* but are no longer invisible to the solver.** `CheckPhysicsCollision` and `GatherPhysicsCells` still skip every `fluidType != None` cell, so §3.3's contact rules are unchanged and a fluid can never produce a contact (`B14`). Since 2026-09-03 the solver additionally runs a **separate** query, `World.GatherFluidContact`, which keeps exactly the cells those two discard and resolves a `Physics.FluidContact` — waterline, current, the fluid's authored coefficients, and its block ID — once per `FixedUpdate`. The ID carries no physics meaning: it is there so a consumer outside the solver can look up a property the struct does not hold, which is how the footstep layer names the fluid it strokes through (`SOUND_ENGINE_DESIGN.md` §5.1) without running a second voxel read that could disagree with the waterline the solver is pushing the body with. Buoyancy, drag and flow push are applied in `CalculateVelocity` alongside gravity, **not** in `ResolveMovement`, which runs once per substep and would scale every force by the substep count. The two scans are deliberately not merged: the collision gather's envelope carries step-height head-room that is meaningless for buoyancy, and the fluid pass answers a different question — it picks the *highest* fluid surface overlapping the body rather than accumulating contacts, then reads that one cell's eight horizontal neighbors on demand for its flow vector. Guarded by `B27`–`B44`, of which `B27` is the positive control — the other 26 baselines stay green whether the fluid path works or does nothing at all. §3.4's **step-up pre-pass** additionally admits a body that is in fluid rather than only one that is `IsGrounded`, and applies `swimStepHeight` rather than `stepHeight` while it does: buoyancy is support for the purpose of climbing out, and the allowance has to be bigger because the float equilibrium parks the feet most of a block below the surface, putting a one-block bank ~1.04 above them — over twice the walking step. The gap is measured from the *surface*, so it does not grow with depth, which is what lets one value clear a one-block bank and still refuse a two-block wall (`B38`). Out of water the pre-pass is additionally gated on the body **asking** to rise, which stepping onto a slab is not: the climb is roughly twice a walking step, and leaving it on contact alone fired it for a body merely falling back into the pool (`B42`). Only a *floating* body gets the allowance — one standing on the bottom of shallow water is walking, and steps like a walker.
- **A falling column's current is *not* the rendered one, and this is the one place the two deliberately diverge.** Everywhere else the push a body feels is the negated meshing flow vector, so physics and visuals agree by construction. A falling voxel breaks that: it stands at full height while the air beside it reports `BurstFluidFlowUtility.DropHeight` (−1.0), so the shared corner derivative saturates *outward* — right for the renderer, which should scroll the texture off the column, and wrong for physics, where it made every waterfall an unwalkable wall pushing the player out horizontally (measured 0.53 units of current against a cliff-backed column, before `pushStrength`). `World.GatherFluidContact` therefore reports **no** current at all for a falling column, and the solver applies a downward pull as a `_verticalMomentum` target instead — on the same axis the swim stroke acts on, so a swimmer can fight it (`B36`) rather than being overridden by it. Pinned by `B35`, whose fixture must stay **asymmetric**: a column standing in open air on all four sides produces four outward corner vectors that cancel exactly in the mean, so a symmetric fixture measures zero horizontal push and passes without the fix.
