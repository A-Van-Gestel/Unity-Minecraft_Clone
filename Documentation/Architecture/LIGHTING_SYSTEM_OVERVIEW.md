...
**Cross-chunk removal veto (Bug 17 fix, July 2026):** a cross-chunk RGB *removal* mod is evaluated per channel against the strongest attenuated blocklight an independent source still supplies the target — an in-chunk neighbor (`CrossChunkLightModApplier.InChunkBlocklightSupport`) or a live third-party cross-chunk neighbor in a chunk other than the emitter (`CrossChunkBlocklightSupport`). A channel a live source still backs is kept, not cleared. This is the RGB mirror of the sky Bug 11/13 veto (§3.7): without it, a removal computed against a stale
snapshot over-cleared an independently-fed channel and the receiver re-lit it, leaving a sourceless over-bright ghost island. An opaque neighbor contributes only its own emission (mirror of `PropagateLightRGB`'s opaque-source arm).

**Removal-node masking (Bug 16 fix, July 2026):** when the RGB darkness wave zeroes a neighbor, the re-enqueued removal node carries the neighbor's pre-zero value **only for the channels actually zeroed** — a kept channel belongs to a different source's gradient. Carrying it made the wave a cross-gradient remover and broke the per-channel strict-decrease termination guarantee: combined with the darkness-phase seam pull-back (§3.6's `CheckEdgeVoxelRGB` restoring border cells to constants), removal nodes could cycle forever inside one job, growing its
queues until OOM.
...
**Runaway fail-safe (July 2026):** the seed and phase loops share a hard work budget (`MAX_BFS_NODES_PER_PASS`, 200k node visits — the heaviest legitimate passes, full-column recalcs across the validation suite, stay well below it). A pass that exceeds it aborts loudly (console error + a dump of the last processed nodes) with `IsStable` false, so the chunk retries — a termination regression (Bug 16's class) degrades to bounded, visible churn instead of unbounded queue growth and an OOM crash.
...
**Output:** The BFS-updated `PaddedLight` volume (its center [2,18) region is extracted back into the center light map on completion, via `ChunkMath.ExtractCenterLight` — LI-2/LI-2b: only the gathered band rows are extracted, band-local source rows to absolute destination rows; outside them the center light map keeps its schedule-time snapshot, which the job provably did not change), the cross-chunk modifications list, and the `IsStable` flag. The voxel data is never modified by the job.
...
2. **Edge checks only ADD, never remove** (§3.6, §4.2). Over-bright stale light at a border is *never* corrected by the edge-check pass — only too-dark light is. The "inverse artifact" (light that refuses to darken) therefore had **no automatic correction path** and persisted until a full relight. Two initiators have since been supplied for its known mechanisms: the Bug 12 cross-seam removal emit (reason 3 below) and the Bug 14 pull-back claim verification (§3.4 step 2 — the dominant planter of border over-bright, a darkness wave's own stale-snapshot
   re-light, is now re-verified against live data at merge time). Over-bright from any *other* origin still has no corrector. Of the three sky mechanisms, RGB blocklight mirrors **one** — the independent-support removal **veto** (`CrossChunkLightModApplier.ComputeBlocklight` per channel, backed by `InChunkBlocklightSupport` / `CrossChunkBlocklightSupport`), added July 2026 to fix **Bug 17** (a sourceless over-bright RGB ghost island left when a stale-snapshot cross-seam removal over-cleared an independently-fed channel and the receiver re-lit it — the
   RGB form of the Bug 11/13 oscillation; archived `_FIXED_BUGS.md` Lighting #22, guarded by baseline B88). The Bug 12 cross-seam removal *initiator* and the Bug 14 pull-back *claim verification* remain **sky-only** — not required for the observed RGB class (the veto alone collapses the oscillation so the removal completes) and deliberately not mirrored.
