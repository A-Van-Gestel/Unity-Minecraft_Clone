...
| "Pause" semantics | `InUI` blocks input only — **no `Time.timeScale` write exists in the project**; fluids, streaming, and block ticks continue while the inventory or console is open. Console adopts the same semantics. **Exception since RF-1 (2026-08-10):** the day/night clock is the one system that holds, and only for the *pause menu* — `World.AdvanceWorldTime` skips its tick while `IsPauseMenuOpen`. Console and inventory do not stop it. Making the pause menu halt the whole simulation is scoped in [`../Design/PAUSE_AND_SIMULATION_HALT.md`](../Design/PAUSE_AND_SIMULATION_HALT.md) (`PA-*`). |
...
| **v2**  | **CMD-6 — `pre-generate-region`** ⬅ **NEW 2026-08-02, needs its own `create-implementation-plan` pass.** Drive every chunk in a selected region to fully generated, programmatically, instead of flying a route and hoping. Selection takes either a **radius around a point** (defaulting to the player) or **four corner coordinates**. ⚠️ **Its first consumer is not a player** — it is the benchmark harness: the P-9 captures repeatedly failed their ≥ 99 % tour-coverage validity gate at vd 26/32 (98.0 % / 97.4 %), so the loading pass measured partly-generation and the latency percentiles were confounded. The current workaround is to run the ensure sweep twice, which is slow and still probabilistic. See [`../Design/CHUNK_PIPELINE_SCHEDULE_QUOTA_THROUGHPUT.md`](../Design/CHUNK_PIPELINE_SCHEDULE_QUOTA_THROUGHPUT.md) §7.0 item 3. Design questions the plan must answer: whether it drives the normal request path under backpressure or bypasses admission entirely; whether it blocks, reports progress, or is cancellable; and how it interacts with the panic gate and the region-file writer at scale. |
...
| `/time [set \| add \| freeze \| resume]` | `WorldTimeManager` (persisted as `worldState.time`)          |   🟢   | **Regrammared by RF-1 (2026-08-10)** from the original `set <0..1>` light-level setter, which existed only because no clock did. Now Minecraft-anchored: 24000 ticks/day, tick 0 = sunrise, `set day\|noon\|sunset\|night\|midnight\|<ticks>`, plus `add` (finally meaningful), `freeze`/`resume` (persisted), and a bare-`/time` query. Still the deterministic-lighting-repro surface                                                                     |
