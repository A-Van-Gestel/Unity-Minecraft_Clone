...
{
    /// <summary>
    /// The routing seam for skylight column recalculations: resolves which chunk's queue bucket a
    /// global voxel-space column belongs to, and the chunk-local column the lighting job's 16×16
    /// heightmap is indexed with. Shared by production (<c>WorldData.QueueSkylightRecalculation</c>,
    ...
    /// </summary>
    public static class SkylightColumnRouting
    {
        /// <summary>
        /// Resolves the voxel-space origin of the chunk that owns the given global column — the
        /// key of the world-level skylight recalculation queue bucket the column is routed to.
