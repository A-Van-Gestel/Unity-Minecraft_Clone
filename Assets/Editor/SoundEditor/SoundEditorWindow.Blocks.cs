...
{
    /// <summary>
    /// <see cref="SoundEditorWindow"/> — the Blocks tab: which clip family every <see cref="SoundMaterial"/>
    /// plays for each event, and the group's playback settings.
    /// </summary>
    /// <remarks>
    /// Clips are assigned by <i>family</i> — the set of numbered variants sharing a name prefix
    /// (<c>impactWood_medium_000..004</c>) — because that is the unit the runtime randomizes over. The
    /// Ambience tab deliberately does not: an ambience bed is one long loop, not a variant set.
    /// </remarks>
    public partial class SoundEditorWindow
    {
        ...
        /// <summary>Draws the Blocks tab: the material list beside the selected material's detail pane.</summary>
        ...
        private void DrawEventRow(BlockSoundGroup group, BlockSoundEvent evt)
        {
            ...
            EditorGUIHelper.PlayStopButton(current, "Audition a random variant, as the game would pick one.",
                PLAY_BUTTON_WIDTH);
