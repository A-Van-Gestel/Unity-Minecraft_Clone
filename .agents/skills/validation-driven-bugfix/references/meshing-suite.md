...
> `IMeshCompletionHost`, and **B34–B36 on a real `Chunk`** (the load-animation toggle regression —
> the suite's first `Chunk`-unit baselines, and its first entry with a `_FIXED_BUGS.md` war story). When adding a scenario, put it in the partial that matches its *unit*,
