...
- **A neighbor's light map is unreadable until that neighbor's VOXEL map exists (MH-13's trap).**
  ...
  "no neighbor chunk", which short-circuits to full sunlight (15) and **never touches the light map** — so a bright reading can mean "the light routing works" *or* "there is no neighbor at all". The API closes the obvious hole (`FillNeighborLight` materializes the chunk; `EnsureNeighborChunk(dir)` does it without brightening, for a deliberately dark neighbor), but the durable discipline is the **all-dark control run**:
  assert the probe emits *no* light before brightening anything, so any un-modelled sunlight source reds instead of passing silently. That is why B40 meshes every leg twice.
