...
| `MeshingValidationSuite.Baseline.cs`         | `B1`–`B11`, `B17` (MH-2 pooled-output stale-data guard), `B22`/`B23` (FL-1/FL-2 sway UV-ZW channels), `B62`/`B63` (FL-4 per-voxel cross-mesh variation and its FL-4b per-block envelope) regression scenarios (must stay green) + probe helpers (`CollectProbeQuads`, `CompareCubeFacesToOracle`, `FindQuadByNormal`, `SectionCellBounds`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| `Framework/SectionRendererTestFixture.cs`    | **MH-6 renderer fixture** (separate from `MeshingTestWorld`): reflection-stub `World.Instance` + 3 distinct dummy materials so `UpdateMeshNative`'s material lookup resolves in edit mode (zero production change); drives the real `SectionRenderer.UpdateMeshNative` and observes via the public `GameObject` (`SharedMaterials`/`MeshBounds`/`IsActive`, plus the settable `OcclusionCulled` → `forceRenderingOff`, MP-5); `Dispose` restores `Instance` + `DestroyImmediate`s                                                                                                                                                                                                                                                                                                                                                                          |
| `Framework/RendererAssert.cs`                | MH-6 renderer assertions: `MaterialsEqual` (ordered, by-reference identity), `BoundsContainAll`/`BoundsContainAllVerts` (containment predicate, no tightness), `BoundsEqual` (exact center+size, MR-4 postcondition)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| `MeshingValidationSuite.CrossChunk.cs`       | `B18`–`B21` cross-chunk border-face culling (MH-10/MH-11) on the opt-in full-`MAP_SIZE` +X neighbor map, plus `B37`/`B38` (MP-7/MH-12) the **permutation guards** — B37 puts one cube per cardinal border each at its own Y, so a map delivered to the wrong slot changes the vertex count; B38 covers the diagonals via fluid corner height (isolate one diagonal, assert the summed vertex height strictly drops); `B39` drives `NeighborMapAssembler.Build` through a marker-returning fake `INeighborMapSource` and asserts all 16 slots; **`B40` (MH-13)** is the same permutation guard for the eight neighbor **light** maps — 8 legs, each an all-dark control run plus a run with exactly one neighbor light map bright, asserting presence-vs-absence of light (never a predicted corner value). Registered via `AddCrossChunkBaselineScenarios` |
...
| `Framework/ChunkLoadAnimationTestFixture.cs` | Edit-mode fixture for a **real `Chunk`** — stubs `World.Instance` + a `Settings` whose `enableChunkLoadAnimations` is settable *between* construction and `TriggerLoadAnimation`. Needs no materials/`BlockDatabase` (the `SectionRenderer` ctor is `Instance`-free); claims the singleton **before** constructing (a `Chunk` ctor reads it, unlike the renderer). Teardown is by hand — `Chunk.Destroy()` uses `Object.Destroy`, an edit-mode error — and destroys the 16 section meshes                                                                                                                                                                                                                                                                                                                                                                  |
| `MeshingValidationSuite.LoadAnimation.cs`    | `B34`–`B36` — the `enableChunkLoadAnimations` toggle regression (`_FIXED_BUGS.md` Chunk Management #08): B34 mid-session enable animates a chunk built while off, B35 the mid-session add is seeded underground (not the world origin), B36 construction-time controls; registered via `AddLoadAnimationScenarios`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
`Jobs.MeshGenerationJob` (the standard-cube + fluid path) runs **for real** — no reimplementation. The harness mirrors the production / `MeshGenerationBenchmark` wiring exactly: default sections (`IsEmpty=false`, `IsFullySolid=false`) force the per-voxel iteration path; light maps + neighbor/custom arrays are constructed-but-empty just as the benchmark leaves them; the water height template is the **real**
...
> apply-path, a **separate** fixture — B12–B14). **MR-* implementation phase Wave 1 (2026-06-18):** MR-3 + MR-4 +
...
> **Since then the suite grew past the job harness — tip is now B63 (59 baselines, 2026-08-27;
...
> `IMeshCompletionHost`, **B34–B36 on a real `Chunk`** (the load-animation toggle regression —
> the suite's first `Chunk`-unit baselines, and its first entry with a `_FIXED_BUGS.md` war story), and MP-7's
> **B37–B38** back on `MeshingTestWorld` (neighbor-map permutation guards — the harness now models all four
> cardinal *and* all four diagonal neighbors via `SetNeighborBlock(CardinalNeighbor|DiagonalNeighbor, …)`,
> each lazily created so an unpopulated direction still behaves as the original empty map; B37 catches
> cardinal swaps via face culling, B38 diagonal swaps via **fluid corner height** — diagonals never reach
> culling; **B39** guards the acquire-site offset table in `Helpers/NeighborMapAssembler`, which feeds the
...
// Cross-seam LIGHT (MH-13). Fill* materializes the neighbor chunk too — see the gotcha below for why:
world.EnsureNeighborChunk(CardinalNeighbor.East);                   // neighbor loaded, all-Air, DARK (controls)
world.FillNeighborLight(CardinalNeighbor.East, packed);             // brighten that one neighbor's light map
world.FillNeighborLight(DiagonalNeighbor.NorthEast, packed);        // diagonals reached only by a CORNER probe
...
- **Renderer no-reassign postcondition (MR-3):** `B15` — prime opaque-only, externally stomp `sharedMaterials` with a sentinel material, then a SAME-bitmask update must leave the sentinel intact (no redundant reassignment). Positive control: a CHANGED bitmask after re-stomping overwrites the sentinel (proving the test can observe a reassignment).
- **Renderer constant-bounds postcondition (MR-4):** `B16` — after a non-empty update, `sharedMesh.bounds` must EQUAL the constant 16³ section cell (center (8,8,8), size (16,16,16)) via `RendererAssert.BoundsEqual`, independent of geometry extent. Positive control: the probe AABB is strictly smaller, so a `RecalculateBounds()`-style tight result would fail the equality.
- **Pooled-output stale-data guard (MH-2 / MR-6):** `B17` — a `MeshOutputPool` instance is rented → run scene A → `Return` (clears) → rented back → run scene B (via `MeshingTestWorld.Run(reuseOutput:)`), then `OutputsEqual` asserts the reused buffer == a fresh-buffer scene B. Guards the MR-6 `ClearForReuse` reset (the job appends + never clears, so an un-reset buffer leaks the prior scene). Positive control: scene A's vert count ≠ scene B's, so the buffer genuinely held foreign data (verified red→green: reset off → `Vertices length 120 != 48`).
...
- **A neighbor's light map is unreadable until that neighbor's VOXEL map exists (MH-13's trap).**
  `SampleNeighborLight` resolves the voxel state first, and a *missing* (length-0) neighbor voxel map means
  "no neighbor chunk", which short-circuits to full skylight (15) and **never touches the light map** — so a bright reading can mean "the light routing works" *or* "there is no neighbor at all". The API closes the obvious hole (`FillNeighborLight` materializes the chunk; `EnsureNeighborChunk(dir)` does it without brightening, for a deliberately dark neighbor), but the durable discipline is the **all-dark control run**:
  assert the probe emits *no* light before brightening anything, so any un-modelled skylight source reds instead of passing silently. That is why B40 meshes every leg twice.
