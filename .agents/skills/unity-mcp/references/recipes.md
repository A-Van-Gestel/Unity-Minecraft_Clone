...
12. **RunCommand namespace blocklist**: scripts using `System.Net`, `System.Diagnostics`, `System.Runtime.InteropServices`, or `System.Reflection` are rejected ("Script was blocked before execution"; an opaque NullReferenceException on an unpatched package). `Newtonsoft.Json` is also unavailable to the dynamic compile. For reflection-heavy or Newtonsoft-dependent editor work, use a temp `[MenuItem]` script driven via `Unity_ManageMenuItem` instead.
