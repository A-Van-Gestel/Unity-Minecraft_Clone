...
10. **Array parameters must be real JSON arrays** (`Types: ["Error"]`, never the string `"[\"Error\"]"`). The embedded package coerces the stringified form, but don't rely on that leniency — it is a local patch, not upstream behavior.
...
14. **`SortedDictionary<,>` does not resolve** in the dynamic compile — it fails with `CS1069: ... forwarded to assembly 'System'`. `Dictionary<,>` and `List<>` are fine; sort after the fact or key by an int.
15. **The editor's culture formats decimals with a comma** — `0.5f.ToString("0.00")` yields `"0,50"`. A probe that builds dictionary keys or comparison strings from formatted floats will silently miss and throw `KeyNotFoundException` partway through, after the useful output has already been appended but before `result.Log` runs, so you lose the whole run. **Key by scaled integers instead** (`Mathf.RoundToInt(value * 4)`), which is also stable across machines. Same trap in reverse when parsing numbers back out of console text.
16. **Project editor types need `global::`** — the dynamic compile wraps `CommandScript` in `Unity.AI.Assistant.Agent.Dynamic.Extension.Editor`, so `Editor.Validation.Framework.X` resolves against that namespace and fails with `CS0234`. Write `global::Editor.Validation.Framework.X`.
