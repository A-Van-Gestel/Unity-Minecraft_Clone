...
8. **DOCUMENT NEW BUGS:** If the bug is not already in `Documentation/Bugs/`, create an entry in the matching category file (`LIGHTING_BUGS.md`, `FLUID_BUGS.md`, `CHUNK_MANAGEMENT_BUGS.md`, …) once the symptom is confirmed real — don't wait for the fix; open bugs must be findable by step 0 of the next session.
    - **Match the target file's existing heading and numbering style** (files differ slightly, e.g. `## Bug NN: Title` vs `## NN. Title`) and continue that file's own numbering — numbers are per-file, not global.
    - Standard fields: `**Severity:**`, `**Status:**` (e.g. `Open`, `Open / Partially mitigated`), `**Files:**` (code pointers), `**Description:**` (symptom + repro conditions), and a root-cause section clearly marked as confirmed vs suspected.
    - If a validation-suite repro scenario exists (or gets written via `validation-driven-bugfix`), name the scenario ID in the entry — that is the established cross-link convention.
    - Only **open** bugs live here; never write an already-fixed bug into a category file (fixed entries belong in `_FIXED_BUGS.md` via the `archive-fixed-bug` skill).
9. **SWITCH TO VALIDATION-DRIVEN FIXING WHERE A SUITE EXISTS:** Once the root cause is understood, check whether the affected system has an editor validation suite (e.g. lighting: `Minecraft Clone/Dev/Validate Lighting Engine`). If so, hand off to the `validation-driven-bugfix` skill — write the deterministic repro scenario first, then fix against it instead of iterating purely on in-game logs.
10. **ARCHIVE ON CONFIRMATION:** After the user confirms the fix works, use the `archive-fixed-bug` skill to move the bug entry to `_FIXED_BUGS.md`. Do not archive pre-emptively.
