...
Adding a mutable `static` field to a MonoBehaviour/manager repeatedly trips Rider's **UDR0004/UDR0005** ("Domain Reload Analyzer"), and Unity's Project Auditor reports the same types as **UAL0010/UAL0013** ("must have an `AutoStaticsCleanup` or `NoAutoStaticsCleanup` attribute"). *Enter Play Mode → Reload Domain* is currently **enabled**, so statics **are** re-initialized between play sessions today: `ProjectSettings/EditorSettings.asset` has `m_EnterPlayModeOptions: 0` (= `None`, i.e. neither domain nor scene reload is disabled), which is what decides it. Re-check that asset rather than a session log — as of 2026-09-17 its `m_EnterPlayModeOptionsEnabled: 1` disagreed with the live editor's `False`, and the options value is the one that matters either way. The rules below are therefore not a workaround for a broken setup: they are what the two analyzers require, and what has to hold before fast enter play mode can ever be turned on (with domain reload off, a stale static leaks straight into the next run). Three rules:

- **Reset every mutable static on play-mode entry.** A field initializer (`= 0`) is not enough — it only runs on domain reload, which fast enter play mode skips. Zero it from code that runs each play start.
- **One `[RuntimeInitializeOnLoadMethod]` per class (UDR0005).** Do NOT add a second `[RuntimeInitializeOnLoadMethod]` reset method to a class that already has one. Fold the reset into the class's existing one (e.g. `World.DomainReset`); only add a fresh `[RuntimeInitializeOnLoadMethod]` in a class that has none. `const` statics are exempt from *this* rule (never reassigned), as are `readonly` and `[ThreadStatic]` members whose value never goes stale — but a `readonly` container's CONTENTS are session state and still need clearing, and a `[ThreadStatic]` reset only ever touches the calling thread's slot, so it is not a reset for the worker threads.
- **Annotate the static for `UAL0010`/`UAL0013`** (`using Unity.Scripting.LifecycleManagement;` — `Unity.Scripting` is already referenced by `Assembly-CSharp`). **No member is exempt here**, unlike the reset rule above: the sweep annotates 8 `static readonly` tables and one `[ThreadStatic]` field, because leaving any flagged member bare re-opens `UAL0013` on its whole type. The attribute goes on the member (field, auto-property or event), which is how Unity's own packages use it:
    - `[NoAutoStaticsCleanup] // reset in DomainReset` — the house default, because Unity's automatic cleanup is `ResetToDefaultValue` and cannot express what the existing resets do (authored defaults like `WorldName = "New World"`, `.Clear()` on a `readonly` container, or `FluidBlockLookup.s_generation`, whose correct reset is an *increment*). Vary the comment when the reason differs (`// immutable table`, `// contents cleared in DomainReset`).
    - `[AutoStaticsCleanup]` — only when `default(T)` *is* the correct fresh value and no reset covers the member. It is also the **opt-in**: Unity generates its `__AutoStaticsCleanup_*` cleanup only for members that carry it, so an unannotated static is left alone and the attributes above document a decision rather than change behavior.
  The reason must name the member's **actual** reset method — the first sweep wrote `DomainReset` everywhere and was wrong in 15 files. Worked example: 92 statics across 45 files, see `@Documentation/Design/PROJECT_AUDITOR_FINDINGS_REPORT.md` §AU-2.
